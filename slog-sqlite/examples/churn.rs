...
//!
//!   cargo run -p slog-sqlite --release --example churn
//!
//! Backend: real S3 when SLOG_TEST_BUCKET is set (AWS_REGION plus env creds),
//! else a local dir at SLOG_BUCKET_DIR (default /tmp/slog-churn-bucket).
//! Knobs: SLOG_CHURN_ROUNDS (30), SLOG_CHURN_TXNS (150), SLOG_CHURN_KEYS
//! (500), SLOG_CHURN_CKPT_EVERY (2), SLOG_CHURN_PROFILE (default |
...
use slog_sqlite::{restore, CheckpointOpts, Db};
...
fn settings_for_profile(profile: &str) -> Option<slatedb::config::Settings> {
    match profile {
        ...
        other => panic!("unknown SLOG_CHURN_PROFILE '{other}' (default|fastflush|compact)"),
    }
}

fn backend() -> (&'static str, Arc<dyn ObjectStore>) {
    if let Ok(bucket) = std::env::var("SLOG_TEST_BUCKET") {
        ...
        let dir = std::env::var("SLOG_BUCKET_DIR").unwrap_or_else(|_| "/tmp/slog-churn-bucket".into());
...
async fn main() -> slog::Result<()> {
    let rounds = env_usize("SLOG_CHURN_ROUNDS", 30);
    let txns = env_usize("SLOG_CHURN_TXNS", 150);
    let keys = env_usize("SLOG_CHURN_KEYS", 500) as u64;
    let ckpt_every = env_usize("SLOG_CHURN_CKPT_EVERY", 2);
    let (backend, bucket) = backend();
    let profile = std::env::var("SLOG_CHURN_PROFILE").unwrap_or_else(|_| "default".into());
