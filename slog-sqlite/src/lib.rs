//! slog-sqlite: a SQLite database whose state lives in, and is reconstructed
//! from, a slog event stream. Committed transactions are captured from the
...
//!
//! Model: one database = one slog stream (`{db}`). The db runs locally in
...
use slog::{CompactionRecord, Error, EventStore, NewEvent, Result, Version};
...
/// unknown types.
pub fn delta_events(e: &slog::Event) -> Result<Vec<Delta>> {
...
/// they're still fetching). Stored as the state of a slog snapshot record.
