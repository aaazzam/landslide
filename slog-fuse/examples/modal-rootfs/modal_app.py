...
Topology (see entrypoint.sh for the full security argument):
  ...
  * Root in the outer context runs `slogfs follow` — the synced replica
...
Prereqs:
  1. Build the CLI against this repo and drop it next to this file:
       docker run --rm -v "$PWD":/w -w /w rust:1-bookworm \
         cargo build --release -p slog-fuse --features fuse --bin slogfs
       cp target/release/slogfs slog-fuse/examples/modal-rootfs/bin-slogfs
  2. Seed a volume (any writer with bucket creds — e.g. write files through
     `slogfs mount` on a Linux box, or your build pipeline uses the Rust API,
     then `slogfs checkpoint <vol>`).
  3. A Modal secret named "aws-creds" holding SLOG_BUCKET + AWS creds
     (AWS_ACCESS_KEY_ID / AWS_SECRET_ACCESS_KEY, and AWS_REGION
     or SLOG_REGION). The agent never sees these: entrypoint.sh scrubs env.

Run:  modal run slog-fuse/examples/modal-rootfs/modal_app.py
"""
...
app = modal.App.lookup("slog-rootfs-demo", create_if_missing=True)

assert BIN.exists(), f"build slogfs first (see docstring); missing {BIN}"
...
image = (
    ...
    .add_local_file(BIN, "/usr/local/bin/slogfs", copy=True)
...
def main():
    sb = modal.Sandbox.create(
        ...
        env={"SLOG_VOL": VOL, "SLOG_MODE": "fuse"},
        timeout=30 * 60,
        experimental_options={"vm_runtime": True},  # real kernel => FUSE
    )
    sb.set_tags({"demo": "slog-rootfs"})
    try:
