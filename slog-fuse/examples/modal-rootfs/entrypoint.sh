...
#
# Env: SLOG_VOL (volume name, required), SLOG_MODE ("fuse" [default; needs
# kernel FUSE, e.g. Modal VM sandboxes] | "mirror" [no FUSE needed, works
# under gVisor]), plus SLOG_BUCKET/AWS_* creds for the object store.
#
...
VOL="${SLOG_VOL:?set SLOG_VOL to the volume name}"
MODE="${SLOG_MODE:-fuse}"
...
case "$MODE" in
    fuse)
        ...
        slogfs follow "$VOL" "$ROOT" >/srv/syncer.log 2>&1 &
        echo $! > /srv/syncer.pid
        for _ in $(seq 1 250); do mountpoint -q "$ROOT" && break; sleep 0.2; done
        mountpoint -q "$ROOT"
        ;;
    mirror)
        slogfs mirror follow "$VOL" "$ROOT" >/srv/syncer.log 2>&1 &
        echo $! > /srv/syncer.pid
        for _ in $(seq 1 250); do [ -f "$ROOT/.slog-mirror.json" ] && break; sleep 0.2; done
        [ -f "$ROOT/.slog-mirror.json" ]
        mkdir -p "$ROOT/proc" "$ROOT/dev" "$ROOT/tmp"
        MERGED="$ROOT" # no overlay: agent uid owns nothing, modes protect files
        ;;
    *) echo "SLOG_MODE must be fuse|mirror" >&2; exit 1 ;;
esac
...
if [ "$MODE" = fuse ]; then
    ...
    mkdir -p /tmp/slogfs-overlay "$MERGED"
    mount -t tmpfs tmpfs /tmp/slogfs-overlay
    mkdir -p /tmp/slogfs-overlay/upper /tmp/slogfs-overlay/work
    mount -t overlay overlay \
        -o "lowerdir=$ROOT,upperdir=/tmp/slogfs-overlay/upper,workdir=/tmp/slogfs-overlay/work" \
        "$MERGED"
