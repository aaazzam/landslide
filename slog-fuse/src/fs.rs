...
use slog::{EventStore, EventStoreReader};
...
/// A slog filesystem behind a FUSE mount. All mutations go through
...
pub async fn mount(
    ...
    let fs = SlogFs::new(Arc::new(tokio::sync::Mutex::new(volume)));
    fuser::mount(fs, mountpoint, &Config::default())
        .map_err(|e| slog::Error::InvalidInput(format!("fuse mount: {e}")))
}
...
/// [`slog::ReaderConfig::options`]) plus `sync_interval`. The kernel is told
...
pub async fn mount_replica(
    ...
    fuser::mount(SlogFs::replica(replica), mountpoint, &config)
        .map_err(|e| slog::Error::InvalidInput(format!("fuse mount: {e}")))
}
