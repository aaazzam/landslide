...
//!
//!   cargo run -p landslide-fuse --features fuse --example churn -- <cmd> [vol]
//!
//! Commands:
...
async fn main() -> landslide::Result<()> {
    ...
    let path = std::env::var("LANDSLIDE_PATH").unwrap_or_else(|_| "landslidefs/churn".into());
    ...
    match cmd.as_str() {
        "mount" => {
            ...
            landslide_fuse::fs::mount(store, bucket, &vol, &mp).await
        }
        "follow" => {
            ...
            landslide_fuse::fs::mount_replica(reader, bucket, &vol, &mp, std::time::Duration::from_secs(2))
                .await
        }
        "verify" => {
