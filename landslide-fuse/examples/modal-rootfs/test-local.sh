...
#
# Run from the repo root:  sh landslide-fuse/examples/modal-rootfs/test-local.sh
set -eu
cd "$(dirname "$0")/../../.."

IMG=landslide-rootfs-test
CTR=landslide-rootfs-test
BUCKET=$(mktemp -d)/bucket
trap 'docker rm -f $CTR >/dev/null 2>&1 || true' EXIT

docker build -f landslide-fuse/examples/modal-rootfs/Dockerfile.test -t $IMG .
...
docker run -d --name $CTR --privileged --device /dev/fuse \
    -e LANDSLIDE_BUCKET_DIR=/bucket -v "$BUCKET":/bucket $IMG sleep infinity >/dev/null
x() { docker exec $CTR "$@"; }

echo "== seed a volume (writable mount, then checkpoint):"
x sh -c 'mkdir -p /seed && landslidefs mount rootfs /seed >/tmp/seed.log 2>&1 & sleep 1.5'
...
x sh -c 'umount /seed; sleep 0.5; landslidefs checkpoint rootfs'
...
boot() {
    ...
    x sh -c "LANDSLIDE_VOL=rootfs LANDSLIDE_MODE=$1 nohup sh /entrypoint.sh >/entrypoint-$1.log 2>&1 &"
...
battery() {
    ...
    x sh -c 'mkdir -p /seed2 && landslidefs mount rootfs /seed2 >/dev/null 2>&1 & sleep 1.5'
