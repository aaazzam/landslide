//! landslide-fuse: a filesystem whose state lives in a landslide event stream.
//!
//! Model: one volume = one landslide stream (`{vol}`). Every mutation (`commit`)
...
use landslide::{CompactionRecord, EventStore, Version};
...
/// current state. Stored as the state of a landslide snapshot record.
