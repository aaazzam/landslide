...
use landslide::{Config, EventStore, EventStoreReader, ReaderConfig};
use landslide_fuse::replica::Replica;
...
async fn replica_rebuilds_from_checkpoint_after_purge() {
    ...
    vol.write_file("/etc/os-release", 0, "landslide".into());
    vol.commit().await.unwrap();
    converge_until(&mut replica, |r| r.image.nodes.contains_key("/etc/os-release")).await;
    assert_eq!(replica.read_file("/etc/os-release").await.unwrap().unwrap().as_ref(), b"landslide");
}
