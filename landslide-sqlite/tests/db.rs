...
use landslide_sqlite::{restore, Db};
...
async fn reopen_fences_the_previous_writer() {
    ...
    // one of two fencing layers: landslide's cooperative stream fence
    ...
    let client_fenced = matches!(
        &err,
        Error::Backend(e)
            if e.kind() == landslide::deps::slatedb::ErrorKind::Closed(landslide::deps::slatedb::CloseReason::Fenced)
    );
