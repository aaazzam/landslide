...
//!
//! Self-skips (passes vacuously) unless `LANDSLIDE_TEST_BUCKET` is set. Uses the
//! standard AWS credential chain via `AmazonS3Builder::from_env`; set
//! `AWS_REGION` as usual.
//!
//! ```sh
//! LANDSLIDE_TEST_BUCKET=my-bucket AWS_REGION=us-east-1 \
//!   cargo test -p landslide-sqlite --test s3 -- --nocapture
//! ```
//!
...
//!
//! ```sh
//! aws s3 rm --recursive s3://$LANDSLIDE_TEST_BUCKET/db/<run> &
//! aws s3 rm --recursive s3://$LANDSLIDE_TEST_BUCKET/ltx/<name>
//! ```
...
fn s3_config(run: &str) -> Result<(Config, Arc<dyn object_store::ObjectStore>), Skip> {
    let Ok(bucket) = std::env::var("LANDSLIDE_TEST_BUCKET") else {
        eprintln!("skipping s3 test: LANDSLIDE_TEST_BUCKET is not set");
        return Err(Skip);
    };
...
async fn replicate_checkpoint_coalesce_rehydrate_on_fresh_vm() {
    ...
    println!("run prefix: db/{run} and ltx/{db_name} in $LANDSLIDE_TEST_BUCKET");
}
...
async fn boot_cost_breakdown() {
    let run = ulid::Ulid::new().to_string();
    let (bucket, _) = match std::env::var("LANDSLIDE_TEST_BUCKET") {
        Ok(b) => (b, ()),
        Err(_) => {
            eprintln!("skipping s3 test: LANDSLIDE_TEST_BUCKET is not set");
            return;
    ...
    let store_nogc = EventStore::open(Config {
        path: path.clone(),
        object_store: m.clone(),
        settings: Some(landslide::deps::slatedb::config::Settings {
